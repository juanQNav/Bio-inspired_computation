...
import datetime
import evals as ev
from utils import gets_data, plotting
import importlib
import inspect
...
def GA(space_sol, eval, eval_args=None, n=10):
    ...
    toolbox.register("individual", tools.initRepeat,
                     creator.Individual, toolbox.attr_float, n=n)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--pop", type=str, required=True,
                    help="Population size in format '(min,max)'")
    ap.add_argument("--pc", type=str, required=True,
                    help="Probability of crossover in format '(min,max)'")
    ap.add_argument("--pm", type=str, required=True,
                    help="Probability of mutation in format '(min,max)'")
    ap.add_argument("--g", type=str, required=True,
                    help="Number of generations in format '(min,max)'")
    ap.add_argument("--ev", type=str, required=False,
                    help="Eval function name")
    ap.add_argument("--sp", type=str, required=True,
                    help="Space solution '(min,max)'")
    ...
    if eval_name == "eval_knapsack":
        weights, values = gets_data.load_knapsack_data(data_path)
        eval_args = [weights, values]
        n = len(weights)
    elif eval_name == "eval_classification":
        data, target = gets_data.load_iris_data(data_path)
        eval_args = [data, target]
        n = 6

    toolb = GA(space_sol=space_sol, eval=eval_function,
               eval_args=eval_args, n=n)
                ...
                for prob_mutation in np.arange(probm_range[0], probm_range[1] + prob_step, prob_step):
                    for generation_size in np.arange(g_range[0], g_range[1] + g_step, g_step):
                        ...
                        for gen in range(int(generation_size)):
                            ...
                            fitnesses = map(
                                lambda ind: toolb.evaluate(ind), invalid_ind)
                        ...
                        if binary:
                            solution = '  '.join(
                                f"{val:.4f}" for val in binarize_ind(hof[0]))
                        ...
                        sumary_configs = sorted(
                            sumary_configs, key=lambda x: x["Best Fitness"])

                        top_two = sumary_configs[:2]

                        remaining_configs = sumary_configs[len(
                            sumary_configs)//2:]
                        num_random = min(4, len(remaining_configs))

                        random_others = random.sample(
                            remaining_configs, num_random)

                        combined_results = top_two + random_others

                        combined_results = sorted(
                            combined_results, key=lambda x: x["Best Fitness"])
    ...
    df.to_csv(os.path.join(
        output_path, f"summary_{date_str}.csv"), index=False)
    ...
    if eval_name == "eval_classification":
        plotting.plot_classification_scatter(
            best_params=best_config['Best solution'], data=data, target=target, date_str=date_str, o=output_path)
